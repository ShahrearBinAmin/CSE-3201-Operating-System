...
void get_input_from_file(process_info processes[],int &total_process,int &time_quanta,priority_queue< pair<int, int>, vector< pair<int,int> >, greater< pair<int,int> > > &blocking_queue)
{
...
void round_robin(double fcfs_tat)
{
    ...
    while (!blocking_queue.empty() || !ready_queue.empty())
    {
        ...
        for(int cur_page=processes[cur_process].current_page;cur_page<processes[cur_process].pages.size();cur_page++)
        {
            ...
            if(processes[cur_process].previous_page_fault==false && pageFaults(processes[cur_process].pages[cur_page],cur_process,processes[cur_process].frame_capacity,cur_page+1))
            {
    ...
    printf("RRS: %.2lf %.2lf",(double)(total_turn_around_time-total_burst_time)/total_process,(double)total_turn_around_time/total_process);
...
void SJF(double fcfc_tat)
{
    ...
    printf("SJF: %.2lf %.2lf",(double)(total_turn_around_time-total_burst_time)/total_process,(double)total_turn_around_time/total_process);
    int total_page_faults=0;
    for(int i=0;i<total_process;i++)
        total_page_faults+=processes[i].page_faults;
...
double FCFS()
{
    ...
    while (!ready_queue.empty())
    {
        ...
        for(int cur_page=processes[pii.second].current_page;cur_page<processes[pii.second].pages.size();cur_page++)
        {
            ...
            if(processes[pii.second].previous_page_fault==false && pageFaults(processes[pii.second].pages[cur_page],pii.second,processes[pii.second].frame_capacity,cur_page+1))
            {
                //cout<<"request: page "<<processes[pii.second].pages[cur_page]<<" process "<<pii.second<<"time "<<cur_time<<endl;
    ...
    printf("FCFS: %.2lf %.2lf",(double)(total_turn_around_time-total_burst_time)/total_process,(double)total_turn_around_time/total_process);
    int total_page_faults=0;
    for(int i=0;i<total_process;i++)
        total_page_faults+=processes[i].page_faults;
